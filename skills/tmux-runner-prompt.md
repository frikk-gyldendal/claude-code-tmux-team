...
**On startup**, load the runtime directory and read the session manifest:
...
1. Load the manifest: `RUNTIME_DIR=$(tmux show-environment CLAUDE_TEAM_RUNTIME 2>/dev/null | cut -d= -f2-) && source "${RUNTIME_DIR}/session.env"`
