...
First, load the runtime directory and source the session manifest:

```bash
RUNTIME_DIR=$(tmux show-environment CLAUDE_TEAM_RUNTIME 2>/dev/null | cut -d= -f2-)
source "${RUNTIME_DIR}/session.env" 2>/dev/null
```

This gives you:
- `SESSION_NAME` — tmux session name (replaces hardcoded session names)
...
RUNTIME_DIR=$(tmux show-environment CLAUDE_TEAM_RUNTIME 2>/dev/null | cut -d= -f2-)
source "${RUNTIME_DIR}/session.env" 2>/dev/null
SESSION="${SESSION_NAME:-$(tmux display-message -p '#S')}"
...
source "${RUNTIME_DIR}/session.env" 2>/dev/null
SESSION="${SESSION_NAME:-$(tmux display-message -p '#S')}"
tmux capture-pane -t "$SESSION:0.X" -p -S -80
```
...
1. Capture full output: `tmux capture-pane -t "$SESSION:0.X" -p -S -80`
